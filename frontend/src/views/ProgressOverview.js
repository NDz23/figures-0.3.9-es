...
class ProgressOverview extends Component {
  ...
  async fetchFullViewData(page = 1) {
    ...
    console.log("Obteniendo data de la API para la pagina: " + page);
    if (results.length > 0) {
  ...
  exportViewToCsv = (data) => {
    const options = {
      ...
      showLabels: true,
      showTitle: true,
      title: 'Progreso de los estudiantes',
    ...
    const csvTestVar = jsonData.map((user, index) => {
      ...
      coursesFilter.forEach((course, i) => {
        ...
        if (userProgress) {
          ...
          singleRecord[course.id] = `Progreso: ${progressPercent}/1 | Secciones: ${sectionsWorked}/${sectionsPossible} | Puntos: ${pointsEarned}/${pointsPossible}`;
        } else {
          ...
          <div className={styles['single-course-progress']}>
            {userProgress ? [
              <div className={styles['data-group']}>
                <span className={styles['data-label']}>Secciones</span>
              ...
              <div className={styles['data-group']}>
                <span className={styles['data-label']}>Puntos</span>
              ...
              <div className={styles['data-group']}>
                <span className={styles['data-label']}>Progreso</span>
    ...
    return (
      <div className="ef--layout-root">
        <HeaderAreaLayout>
          <HeaderContentStatic
            title='Progreso de los estudiantes'
            subtitle={'Puedes ver una instant&aacute;nea del progreso de los alumnos. N&uacute;mero de resultados: ' + this.state.count + '. Tambien puedes filtrar los resultados en la vista y luego exportar los datos como un archivo CSV.'}
          />
        </HeaderAreaLayout>
        {this.state.csvExportProgress ? (
          <div className={cx({ 'container': true, 'csv-export-content': true})}>
            {(this.state.csvExportProgress < 1) ? [
              <h2>Exportando CSV...</h2>,
              <p>No cierres esta pestaña de tu navegador.</p>
            ] : [
              <h2>Exportación exitosa!</h2>,
              <p>Dependiendo de la configuraci&oacute;n de su navegador, se le pedir&aacute; que guarde el archivo generado o el archivo se descargar&aacute; autom&aacute;ticamente en su carpeta de Descargas predeterminada.</p>,
              <p>Ya puedes cerrar el exportador.</p>,
            ]}
          ...
          <div className={cx({ 'container-max': this.state.wideView, 'container': !this.state.wideView, 'users-content': true})}>
            <div className={styles['refining-container']}>
              ...
              {(this.state.selectedCourses.length || this.state.searchQuery) ? (
                ...
                >
                  Generar CSV de la vista actual
                </button>
              ) : ''}
            </div>
            {this.state.pages ? (
              <div className={styles['view-controls-container']}>
                ...
                >
                  {this.state.wideView ? 'Cambiar a vista estrecha' : 'Cambiar a vista amplia'}
                </button>
              </div>
            ) : ''}
            {(this.state.selectedCourses.length || this.state.searchQuery) ? (
              ...
              <div className={styles['no-data-message']}>
                Ingrese un t&eacute;rmino de b&uacute;squeda y / o seleccione curso (s) para mostrar los datos.
              </div>
            )}
            {this.state.pages ? (
              <Paginator
